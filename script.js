...
const shouldUseCursor = !isTouchDevice() && window.innerWidth > 968;
...
if (shouldUseCursor) {
    ...
    // Only proceed if all cursor elements exist
    if (cursorDot && cursorRing && cursorRingOuter) {
        let mouseX = 0;
        let mouseY = 0;
        let dotX = 0;
        let dotY = 0;
        ...
        console.log('%c✨ Custom cursor initialized', 'color: #64FFDA; font-weight: bold;');
    } else {
        console.warn('Custom cursor elements not found in DOM');
    }
} else {
    console.log('%c📱 Custom cursor disabled (touch device or small screen)', 'color: #8892B0;');
}
